...
if errorlevel 1 exit /b %errorlevel%
gcc -std=c11 -O2 -Wall -Wextra -Werror -municode tests\core.c src\core\workspace.c src\core\launch_queue.c src\platform\shell_icons.c src\ui\hold_drag.c src\ui\file_manager.c src\platform\storage.c build\sqlite3.o build\nova-resource.o -o build\core-test.exe -ldwmapi -lcomdlg32 -lcomctl32 -luxtheme -lshell32 -lole32 -ladvapi32 -lgdi32 -luser32 -luuid -lshlwapi
if errorlevel 1 exit /b %errorlevel%
build\core-test.exe %*
exit /b %errorlevel%
