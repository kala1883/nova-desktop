...
#define LEGACY(section,key,value) assert(WritePrivateProfileStringW(section,key,value,config_path))
    ...
    collection->count=2;collection->tasks[1]=collection->tasks[0];lstrcpyW(collection->tasks[1].name,L"构建项目");lstrcpyW(collection->tasks[1].command,L"npm test");collection->tasks[1].mode=BATCH_PARALLEL;
