...
- 集合保存在 SQLite `batch_tasks` scope，带 Version/Count 和逐任务字段；保存整组配置使用一个事务。首次加载将旧 `batch_git_pull_main` 配置迁移成第一条，保留旧 scope。缺失字段、无效模式、重复目录、超长字符串、超容量与更新版本数据会拒绝加载，避免覆盖。空集合有明确版本标记，不会重新迁移旧任务。
