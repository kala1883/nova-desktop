...
src/
  ...
  ui/
    batch_tasks.[ch]        设置入口、目录管理、逐项状态与取消待执行项
...
- 批量任务最多 16 条，每条保存名称、命令（2047 字符）、目录（24 个）与顺序/并行模式。UI 线程拥有集合与窗口，同一时刻执行一条任务。协调线程持有不可变快照；顺序模式逐目录等待，并行模式最多创建 24 个目录工作线程，各自启动一个受 Job Object 管理的系统 `cmd.exe`，目录单独传入。进度回调只投递消息，所有窗口状态更新仍在 UI 线程。取消只跳过未开始项；协调线程在所有目录线程结束后汇总结果并释放句柄。
